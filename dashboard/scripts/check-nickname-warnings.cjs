...
assert(page.includes('Глобальні структури серверного ніку') && page.includes('VALID_NICKNAME_STRUCTURES.map'), 'Discord settings UI must show the fixed global structures instead of an arbitrary template editor');
...
assert(page.includes('/api/dashboard/discord/nicknames/test-message') && page.includes('Тест повідомлення'), 'owner UI must expose a private nickname-warning test button');
...
assert(warnings.includes('Допустимі формати:') && warnings.includes('Імʼя [Мейн, Альт1, Альт2]'), 'warning text must explain all globally valid nickname structures');

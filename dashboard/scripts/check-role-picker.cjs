...
const checks = [
  ...
  [compactRolePicker.includes('--role-picker-card-height: 40px;'), 'desktop role cards keep a compact bounded height'],
  [compactRolePicker.includes('grid-template-areas: "control body badge";'), 'role card content stays on one stable horizontal row'],
  [compactRolePicker.includes('@media (max-width: 640px)') && compactRolePicker.includes('--role-picker-card-height: 44px;'), 'mobile role cards use the compact height variant'],
  ...
  [compactRolePicker.includes('.discord-role-selected {') && compactRolePicker.includes('min-height: 22px;'), 'selected-role summary is compact'],
  [compactRolePicker.includes('.discord-role-search-shell {') && compactRolePicker.includes('min-height: 36px;'), 'role search field is compact'],

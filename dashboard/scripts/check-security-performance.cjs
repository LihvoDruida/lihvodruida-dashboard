...
ok(!dockerfile.includes("require(pkg+'/package.json')") && dockerfile.includes("path.join(process.cwd(),'node_modules',pkg,'package.json')"), 'Docker overlay verifier must read package manifests from node_modules without relying on package.json subpath exports');
...
ok(securityAuditScript.includes('PasswordAuthentication') && securityAuditScript.includes('docker inspect --format') && !securityAuditScript.includes('out=\"$(docker compose port') && securityAuditScript.includes('backup files are private'), 'read-only host security audit must cover SSH, actual Docker host bindings, backups and secret permissions without Compose v5 false positives');
ok(securityAuditScript.includes('addr ~ /^127\\./') && securityAuditScript.includes('sudo make security-audit') && securityAuditScript.includes('SUDO_USER'), 'host audit must ignore loopback-only listeners and distinguish privilege-limited UFW/SSH checks');
ok(hostHardenScript.includes('CONFIRM_SSH_KEY_WORKS') && hostHardenScript.includes('00-mistblossom-hardening.conf') && hostHardenScript.includes('PasswordAuthentication no') && hostHardenScript.includes('sshd -T') && hostHardenScript.includes('ufw --force delete allow') && hostHardenScript.includes('Nginx Full') && hostHardenScript.includes('ufw limit OpenSSH') && hostHardenScript.includes('fail2ban'), 'host hardening helper must refuse unsafe SSH lockout, verify effective sshd policy, remove generic public web allows, and enable firewall/brute-force controls');

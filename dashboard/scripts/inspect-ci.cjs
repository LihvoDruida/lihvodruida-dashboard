...
warn(/"build:ci"\s*:\s*"npm run cleanup:legacy && npm run typecheck && npm run check:actions && npm run check:site && npm run check:rules-accept && npm run check:poll-scheduling && npm run check:imports && npm run audit:styles && npm run audit:ui && npm run inspect:ci && npm run build"/.test(packageJsonText), 'build:ci should clean stale overlay files before typecheck, API-action, site-navigation, rules-onboarding, poll-scheduling, style/UI audits, inspect-ci and build gates.');

...
warn(/"build:ci"\s*:\s*"npm run cleanup:legacy && npm run typecheck && npm run check:actions && npm run check:site && npm run check:rules-accept && npm run check:poll-scheduling && npm run check:admin-overview && npm run check:footer-version && npm run check:raid-editor && npm run check:discord-management && npm run check:account-cleanup && npm run check:imports && npm run audit:styles && npm run audit:ui && npm run inspect:ci && npm run build"/.test(packageJsonText), 'build:ci should clean stale overlay files before typecheck, API-action, site-navigation, rules-onboarding, poll-scheduling, admin-overview, footer-version, raid-editor, Discord-management, account-cleanup, import, style/UI, inspect-ci and build gates.');

...
warn(/"build:ci"\s*:\s*"npm run typecheck && npm run check:actions && npm run inspect:ci && npm run build"/.test(packageJsonText), 'build:ci should keep typecheck, API-action contract, inspect-ci and build gates.');

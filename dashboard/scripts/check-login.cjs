...
const checks = [
  ...
  ["WoW-inspired frame and corner treatments exist", page.includes("login-shell__corner--tl") && page.includes("login-title-divider") && css.includes(".login-shell__corner") && css.includes("rgba(221, 169, 76")],
  ...
  ["login security state is visible without adding another form", page.includes("Discord SSO") && page.includes("Role Sync") && page.includes("серверна сесія")],
  ...
  ["mobile login layout has a dedicated breakpoint", css.includes("@media (max-width: 640px)") && css.includes(".login-trust-row") && css.includes("grid-template-columns: 1fr")],

...
const checks = [
  ...
  ['final PNG uses lossless adaptive-filter compression', /const WELCOME_CARD_PNG_OPTIONS = \{[^}]*compressionLevel: [7-9],[^}]*adaptiveFiltering: true,[^}]*palette: false,/.test(card) && card.includes('.png(WELCOME_CARD_PNG_OPTIONS)')],
  ['final PNG never enables lossy palette quantization', !/\.png\(\{[^)]*\b(quality|colou?rs|dither|effort)\b/.test(card) && !/WELCOME_CARD_PNG_OPTIONS = \{[^}]*\b(quality|colou?rs|dither|effort)\b/.test(card)],
  ['welcome background asset stays losslessly optimized (<= 2.6 MB)', fs.statSync(path.join(root, 'public/assets/discord-welcome-card-night-elf-base.png')).size <= 2_600_000],

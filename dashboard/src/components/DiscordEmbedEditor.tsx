        ...
        {buttons.map((button, index) => (
          <article className="discord-autorole-card" key={button.id}>
            ...
            <div className="discord-autorole-grid">
              ...
              <label className="content-field"><span>Discord-роль</span><select className="select modern-select" value={button.roleId} onChange={(e) => patch(button.id, { roleId: e.currentTarget.value })}><option value="">Вибери роль</option>{roles.map((role) => <option key={role.id} value={role.id} disabled={role.manageable === false}>@ {role.name}{role.manageable === false ? " • недоступна" : ""}</option>)}</select>{button.roleId && roles.find((role) => role.id === button.roleId)?.manageable === false ? <small className="discord-json-error">{roles.find((role) => role.id === button.roleId)?.blockedReason || "Ця роль зараз недоступна для керування ботом."}</small> : <small>Бот може видавати тільки ролі нижче своєї найвищої ролі та за наявності Manage Roles.</small>}</label>
...
function DiscordEmbedEditorInner({
  ...
  const autorolesValid = !isAutoroles || (autoroleButtons.length > 0 && autoroleButtons.every((button) => {
    const role = roles.find((item) => item.id === button.roleId);
    return Boolean(button.label.trim()) && /^\d{16,25}$/.test(button.roleId) && Boolean(role) && role?.manageable !== false;
  }));

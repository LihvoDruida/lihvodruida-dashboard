...
|---------|----------|--------|
| щохвилини | `/api/raids/lifecycle` | закриття запису за дедлайном/на старті, 15-хв нагадування, retry Discord-кнопок і cleanup повідомлень |
...
| щогодини | `/api/raids/lifecycle?sweep=1&limit=100` | bounded recovery sweep для пропущених lifecycle-дій після збою |
| кожні 15 хв | `/api/dashboard/profiles/orphan-cleanup` | scheduler перевірки/очищення акаунтів за збереженими налаштуваннями |
| кожні 15 хв | `/api/dashboard/discord/nicknames/automation` | scheduler перевірки ніків і попереджень |

Логіка рейду після публікації:

- якщо увімкнено закриття запису за N хвилин — у дедлайн статус стає `closed`, сайт і Discord блокують усі signup-кнопки;
- якщо окремий дедлайн вимкнено — те саме відбувається рівно в час старту рейду;
- за 15 хвилин до старту в тому самому Discord-каналі створюється окреме нагадування;
- reminder видаляється через 4 години після старту;
- основне Discord-повідомлення рейду зберігається 24 години після старту, після чого видаляється;
- якщо Discord був недоступний у момент закриття, scheduler повторює синхронізацію кнопок на наступних тіках;
- щохвилинний прохід використовує вузьке вікно дат, а повний recovery sweep виконується лише щогодини.
